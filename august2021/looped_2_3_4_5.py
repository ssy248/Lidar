# July 20  looped 2,3,4,5 method

#  test one starting cluster (at a time) : 0.5 sec map
...
for initialcluster in range(c1, c2+1):
    ...
    for i in range(initialframe, endframe+1):
        ...
        with open(name) as csv_file:
                ...
                for c in currentmap3:
                    ...
                    if dist < thres:
                        ...
                        if dist< mindist:
                            ...
                            mcx = cx
                            mcy = cy
                            # Set prevmap here, July 19 
                            prevmap = totalmap[minclust]
                            # print
                            print("total map july 19 is", totalmap)
                if foundmin==1:
                    #print("found min")
                    ...
                    if diff_tot < 3: # cheange from 5
                        print("diff tot small < 3, do not chec3k angles")
                        print("minclust is", minclust)
                        # set the rest of the params
                        prevmap= totalmap[minclust]
                        print("diff tot small prevmap is", prevmap)
                        avx = mcx
                        avy =mcy
                        finalarray.append(minclust)
                        finalx.append(mcx)
            ...
            # fromlist
            print("fromlist is currently,", fromlist)
            print("go forward is ", goforward)
            print("prevmap is", prevmap)"""

            # update fromlist if current cluster not 0 , July 18
            lastentry = finalarray[-1]
            if lastentry != 0:
                print("last entry not zero")
                # update prevmap
            ...
            # count the # of zeros at the end of array
            ifa = len(finalarray)-1
            ct =0
            while ifa>=0:
                if finalarray[ifa]!=0:
                    break
    ...
    alen = len(finalarray)
    print("length array,", alen)
    print("length of array minus 6", alen-6)
    ...
    # print if there is jump : if # zeros is > 6
    ct0 = countX(finalarray,0)
    print("count of 0s is:", ct0)
